...
const jwtSecret = 'dsfaerwdsvfearg'; 
...
const photoMiddler = multer({dest:'uploads/'})
app.post('/upload', photoMiddler.array('photos', 100), (req, res) =>{
  const uploadedFiles =[];
...
app.post('/bookings', async (req, res) => {
  ...
  Booking.create({
    place, checkIn, checkOut, noOfGuests, name, phone, price, user: userData.id,
  }).then((doc) => {
    res.json(doc);
  }).catch((err) => {
    throw err;
